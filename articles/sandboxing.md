...
- When you want to instantiate a non-networked GameObject, use `Sandbox.LocalInstantiate` instead of `GameObject.Instantiate`. `Sandbox.LocalInstantiate` respects the running sandboxes, so when a hidden sandbox instantiates a new GameObject, it will not be visible because that sandbox is hidden.
